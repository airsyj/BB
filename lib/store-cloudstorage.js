...
async function ensureLoaded() {
  ...
  if (!loadPromise) {
    loadPromise = (async () => {
      // 读取失败重试最多 3 次，避免因瞬时网络/凭据问题把缓存永久卡成空
      let lastErr = null;
      ...
      // 重试后仍失败：以空数组兜底，但保留 loadPromise 可重试（不清空缓存标记由调用方决定）
      cacheReports = cacheReports || [];
      cacheNotices = cacheNotices || [];
      cacheLoaded = true;
      if (lastErr) console.error('[store] 初始加载失败:', lastErr.message);
    })();
  }
  return loadPromise;
}
...
async function add(report) {
  return withLock(async () => {
    // 始终基于云端最新内容追加，杜绝多实例/并发时各自拿着旧缓存互相覆盖、丢记录
    const arr = await loadReportsRaw();
    arr.push(report);
    cacheReports = arr;
    cacheLoaded = true;
...
// 整文件替换（供一次性编号迁移使用：先改内存对象，再整体落盘）
async function replaceAll(arr) {
  return withLock(async () => {
    cacheReports = Array.isArray(arr) ? arr : [];
    cacheLoaded = true;
    await persistReports();
    return cacheReports;
  });
...
module.exports = {
  ...
  ownerOf, mine, update, conflicts, replaceAll,
